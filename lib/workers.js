...
workers.log = function(originalCheckData,checkOutcome,state,alertWarranted,timeOfCheck){
  ...
  var logFileName = originalCheckData.checkId;

  // append the log sting to file
...
workers.rotateLogs = function(){
  ...
  // false indicates that we don't want to include compressed files as well
  _logs.list(false,function(err,logs){
    if(!err && logs && logs.length > 0){
      logs.forEach(function(logName){
        // compress the data to a different file
        // remove .log extension name
        // uncompressed logs will now have a .log extension, compressed ones will have a date timestamp as their extension
        var logId = logName.replace('.log','');
        var newFileId = logId+'-'+Date.now();
        _logs.compress(logId,newFileId,function(err){
          if(!err){
            // truncate the log - empty out the contents of the original log file after we've moved the contents to a new file
...
// timer to execute to execute the log rotation process once per day
