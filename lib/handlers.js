              ...
              if(!err){
                _data.read('users',checkData.userPhone,function(err,userData){
                  if(!err && userData){
                    ...
                    // remove the specified checks from their list of checks on the user object by chcecking the index of the check on the user obect
                    ...
                    if(checkPosition > -1){
                      ...
                      callback(500, {'Error' : 'Could not find the check on the users object, user check was not removed'});
                    }
                ...
                callback(500, {'Error' : 'Could not find the user who created the check and could not remove the check from the list of checks on user object'});
              }
            });
          } else {
            callback(403, {'Error' : 'Missing required token in header, or token is invalid'});
